...
int main(int argc, char* argv[2]){
    ...
    if(argc < 2){
        std::cout << "|   Unknown command, or path   |";
        return 3;
    }
    if(strcmp(argv[1], "-v") == 0){
        ...
        std::cout << "|Version 1.6.2206.2025 | Mi 1.5.1206.2025 adde|\n";
...
//|     OPEN FILE      |
    ...
    program.open(argv[1]);
    if(!program.is_open()){      std::cout << "|   Unknown path, or command   |";
                                 return 1; }
...
//|   INTERPRETATION   |
    ...
    while (i != std::size(code)){
        ...
        case ')':
            ...
            break;
        case '?':
            if (std::atoi(std::string{code[i+1],code[i+2]}.c_str()) != memory[ptrY][ptrX])    i += 4;    else
            if (code[i+1] == '.' && memory[ptrY][ptrX+1] == memory[ptrY][ptrX])               i += 4;    else
            if (code[i+1] == ',' && memory[ptrY][ptrX+1] == memory[ptrY][ptrX])               i += 4;
            break;
        case '|':
            if(code[i+1] == '.')        memory[ptrY][ptrX] |= memory[ptrY][ptrX+1];
            else if(code[i+1] == ',')   memory[ptrY][ptrX] |= memory[ptrY][ptrX-1];
